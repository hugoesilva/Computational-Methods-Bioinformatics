...
pseudo_valence_angle_bottom = 62
...
def createChain(angles, direction, visited, left_chain, right_chain, chains):
    ...
    first_left_chain = left_chain.copy()
    first_right_chain = right_chain.copy()

    second_left_chain = left_chain.copy()
    second_right_chain = right_chain.copy()
    ...
    if direction == "left":
        for combination in angles[last_left_atom]:
            ...
            if new_element in visited:
                continue


            first_left_chain.append(new_element)
            second_left_chain.append(new_element)

            createChain(angles, "right", visited.copy() + [new_element], first_left_chain, first_right_chain, chains)
            #createChain(angles, "left", visited.copy() + [new_element], second_left_chain, second_right_chain, chains)

            chains.append(first_left_chain[::-1] + first_right_chain[1:])
            chains.append(second_left_chain[::-1] + second_right_chain[1:])
    else:
        for combination in angles[last_right_atom]:
                ...
                first_right_chain.append(new_element)
                second_right_chain.append(new_element)

                createChain(angles, "left", visited.copy() + [new_element], first_left_chain, first_right_chain, chains)
                #createChain(angles, "right", visited.copy() + [new_element], second_left_chain, second_right_chain, chains)
    ...
    for atom in angles:
        ...
        for chain in chains:
            if len(chain) > max_len:
                ...
                max_len_chains = [chain.copy()]
            elif len(chain) == max_len:
                max_len_chains.append(chain.copy())
        break
...
#217, 243, 294, 301, 266, 256, 237, 203, 177, 160, 117, 83, 69, 63, 39, 19, 4, 3, 16, 27, 54, 72, 70
